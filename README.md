...
(167 mislabeled, 13 ultra-hard, and 372 very-hard test positives).
...
complete test set. The exact utterance lists removed at each level are
frozen in the code repository (`audit_lists/`); a few of these
utterances carry a different label in the census files below, which
reflect later verification rounds. The full census, described in the
paper as ongoing, has since been completed and is released here. It
identified additional problematic utterances beyond the automatically
flagged set: over all 21,282 test positives, the final labels are
347 `bad`, 40 `ultra_hard`, 561 `very_hard`, and 1,175 `hard`.
...
| `v25ce_train_misses_kw1_verdicts_final.csv` | Final verdicts for the 118 kw1 training-set misses of the V2.5 logistic head, merged from a two-pass listening audit. |
...
2. **Human listening over the flagged utterances**: two passes
   ...
   marked 167 positives as mislabeled, 13 ultra-hard, and 372 very-hard,
...
3. **Train-miss audit (kw1).** The 118 training-set misses of the V2.5
   ...
   (`v25ce_train_misses_kw1_verdicts_final.csv` holds the merged final verdicts).
...
implementing and reviewing the flagging and listening scripts in this
repository, following the author's specifications, and with editing this
README. All listening judgments and annotations were made by the author.
